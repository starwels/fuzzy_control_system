 ...
 */
	...
	if (clientID!=-1)
	{
		...
		while (simxGetConnectionId(clientID)!=-1)
		{
			 ...
			 */
			// simxAuxiliaryConsolePrint(clientID,consoleHandle,"olha a distancia\n\n",simx_opmode_blocking);
                        for(int i=0; i<6; i++){
				...
				{//calculating distance sqrt(x^2+y^2+z^2)
					...
					// simxAuxiliaryConsolePrint(clientID,consoleHandle,std::to_string(distances[i]).c_str(),simx_opmode_blocking);
					// simxAuxiliaryConsolePrint(clientID,consoleHandle,"; ",simx_opmode_blocking);

					if(distances[i]>999) distances[i] = 999;
